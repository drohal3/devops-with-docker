...
```
## Solution
Dockerfile: https://github.com/nerdalert/Dockerfiles/blob/master/nmap/Dockerfile
Built the image by myself, Dockerfile is in [nmap](./nmap) directory
```
docker build -t nmap .
```
Running the scan:
...
Output:
```
Starting Nmap 7.94 ( https://nmap.org ) at 2024-02-05 11:09 UTC
Nmap scan report for localhost (127.0.0.1)
Host is up (0.0000040s latency).
Other addresses for localhost (not scanned): ::1
Not shown: 996 closed tcp ports (reset)
...
Removed "ports" part from backend and frontend in docker-compose.yaml
The containers can still communicate with each other through the docker network

Running nmap after rebuilding the app:
Output:
```
Starting Nmap 7.94 ( https://nmap.org ) at 2024-02-05 11:22 UTC
Nmap scan report for localhost (127.0.0.1)
